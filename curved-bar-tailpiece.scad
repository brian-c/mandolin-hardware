...
bevel = sqrt(2)/2;
outer_d = 80;
leg_d = screw_head_d+4;
middle_d = outer_d-bar_d;
...
screw_length = inner_length+leg_d+pin_gap*2;
...
difference() {
  union() {
    ...
    translate([0, middle_d/2, 0]) {
      difference() {
        intersection() {
          rotate_extrude($fn=128) translate([middle_d/2, 0, 0]) circle(d=bar_d);
    ...
    translate([0, middle_d/2, (bar_d-pin_d)/-2]) {
      intersection() {
        for (i=[(strings-1)/-2:(strings-1)/2]) {
          rotate([90, 0, i*(pin_angle/strings)]) translate([0, 0, middle_d/2]) cylinder(d=pin_d, h=pin_h+bar_d/2, $fn=32);
        }
        translate([0, 0, pin_h/4]) resize([outer_d+pin_h*2, outer_d+pin_h*2, bar_d*1.5]) sphere(d=1, $fn=128);
      }
    }
  ...
  translate([0, middle_d/2, 0]) for (i=[(strings-1)/-2:(strings-1)/2]) {
    rotate([90, 0, i*(pin_angle/strings)]) translate([0, 0, middle_d/2]) cylinder(d=bevel*1.2, h=leg_d, center=true, $fn=4);
  }
